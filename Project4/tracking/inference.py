    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for p in self.legalPositions:
                ...
                self.beliefs[p] = emissionModel[trueDistance] * last[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                new[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [legal[i % len(legal)] for i in range(num)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for particle in self.particles:
                prob[particle] += emissionModel[util.manhattanDistance(pacmanPosition, particle)]

            prob.normalize()

            if prob.totalCount() == 0:
                self.initialize(gameState)
            else:
                self.particles = [util.sample(prob) for i in range(self.numParticles)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        self.particles = [product[i % len(product)] for i in range(self.numParticles)]
    ...
    def observeState(self, gameState):
        ...
        self.dementor(noisyDistances)

        prob = util.Counter()

        for particle in self.particles:
            prob[particle] += reduce(operator.mul,
                                     [1] + [emissionModels[i][util.manhattanDistance(particle[i], pacmanPosition)]
                                     for i in range(self.numGhosts) if noisyDistances[i] is not None])
        prob.normalize()
        if prob.totalCount() == 0:
            self.initializeParticles()
            self.dementor(noisyDistances)
    ...
    def dementor(self, noisyDistances):
        for i in range(self.numGhosts):
